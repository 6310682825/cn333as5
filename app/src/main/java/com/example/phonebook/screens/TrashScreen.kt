...
private fun TrashTopAppBar(
    ...
    TopAppBar(
        title = { Text(text = "To Be Deleted", color = MaterialTheme.colors.onPrimary) },
        navigationIcon = {
...
private fun mapDialogTitle(dialog: Int): String = when (dialog) {
    RESTORE_PHONES_DIALOG -> "Do you want to restore selected phone lines?"
    PERMANENTLY_DELETE_DIALOG -> "Do you want to delete selected phone lines?"
    else -> throw RuntimeException("Dialog not supported: $dialog")
}

private fun mapDialogText(dialog: Int): String = when (dialog) {
    RESTORE_PHONES_DIALOG -> "Are you sure you want to restore selected phone lines?"
    PERMANENTLY_DELETE_DIALOG -> "Are you sure? This action is irreversible"
    else -> throw RuntimeException("Dialog not supported: $dialog")
}